...
const selectorClass =  background.length === 3 && background.reduce((a, b) => a + b, 0) < 1.5
     ? 'selector dark'
     : 'selector light';
...
const addDataSetButton = document.createElement('div');
...
function createPipeline(fileName, fileContents) {
  ...
  presetSelector.innerHTML = vtk.Rendering.Core.vtkColorTransferFunction.vtkColorMaps.rgbPresetNames
    .map(
      (name) =>
        `<option value="${name}" ${
  ...
  function applyPreset() {
    const preset = vtk.Rendering.Core.vtkColorTransferFunction.vtkColorMaps.getPresetByName(presetSelector.value);
  ...
  function updateColorBy(event) {
    ...
    let colorMode = vtk.Rendering.Core.vtkMapper.ColorMode.DEFAULT;
    let scalarMode = vtk.Rendering.Core.vtkMapper.ScalarMode.DEFAULT;
    ...
    if (scalarVisibility) {
      ...
      scalarMode =
        location === 'PointData'
          ?  vtk.Rendering.Core.vtkMapper.ScalarMode.USE_POINT_FIELD_DATA
          :  vtk.Rendering.Core.vtkMapper.ScalarMode.USE_CELL_FIELD_DATA;
